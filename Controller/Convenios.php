...
//mostrando la lista de convenios  
    ...
    {  
        ...
        {   
        $data['page_id'] = 8;
        $data['page_title'] = "Pagina Convenios";
        $data['page_name'] = "Mostrando Convenios"; 
        $data['page_functions_js']="funciones_convenios.js";
        $this->views->getView($this, "Listar", $data);
        }

        public function mostrarConvenios()
        { 
            $arrData = $this->model->selectConvenios();
            ...
            {             
                ...
                {
                    $btn_editar = '<button class="btn btn-primary btn-sm"  onClick="fntEditar('.$arrData[$i]['id_convenio'].')" title="Editar"><i class="fas fa-pencil-alt"></i></button>';
                }
                if($_SESSION['permisosMod']['d'])
                {
                    $btn_eliminar = '<button class="btn btn-danger btn-sm" onClick="fntEliminar('.$arrData[$i]['id_convenio'].')" title="Eliminar"><i class="fas fa-trash-alt"></i></button>';
                }
                if($_SESSION['permisosMod']['r'])
                {
                    $btn_ver = '<button class="btn btn-warning btn-sm" onClick="fntVer('.$arrData[$i]['id_convenio'].')" title="Ver"><span class="bi bi-eye-fill"></span></button>';
                }
        ...
        {  
            ...
            $codigo =  $this->model->ObtenerCodigoConvenio();
            ...
            if(empty($idConvenio))
            {
                // IMPLEMENTANDO LA ALERTA VACIO 
                if(empty($codigo) || empty($empresa) || empty($tipoConvenio) || empty($fechaInicio) || empty($fechaFinal) || empty($new_name_file1))
                {
                    $alert='vacio';
                }
        ...
        {   
            $arrData = $this->model->selectConveniosEliminados();
            for($i=0; $i<count($arrData); $i++)
            {   if($arrData[$i]['status']==1)
                {
