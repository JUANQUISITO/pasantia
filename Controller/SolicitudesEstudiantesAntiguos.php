<?php 
ob_start();
class SolicitudesEstudiantesAntiguos extends Controllers
{
    ...
    public function solicitudesestudiantesantiguos() 
    {
        $data['page_name']="solicitud_estudiantes_antiguos";
        $data['page_functions_js']="funciones_solicitud_estudiantes_antiguos.js";
        
        $this->views->getView($this, "Listar",$data);
    }


    public function getSolEstAntiguos()
    {   
        $arrData = $this->model->selectSolicitudesEstudiantesAntiguos();
        ...
        {  
            ...
            {
                $arrData[$i]['status'] = '<span class="badge badge-success">Nuevo</span>'; 
            }
            else 
            {
            ...
            {
                $btn_editar = '<button  class="btn btn-secondary btn-sm"  onClick="recibirEstudiante('.$arrData[$i]['id_sol_est_antiguo'].')" title="Verifcar"><i class="fas  fa-cloud-download-alt"></i></button>';
            }
            // if($_SESSION['permisosMod']['d'])
            // {
            //     $btn_eliminar = '<button class="btn btn-danger btn-sm" onClick="EliminarEmpresas('.$arrData[$i]['id_sol_est_antiguo'].')" title="Eliminar"><i class="fas fa-trash-alt"></i></button>';
            // }
    ...
    public function insertar()
    {
        ...
        $data = $this->model->selectSolicitudesEstudiantesAntiguos();
        header("location: " . base_url() . "SolicitudesEstudiantesAntiguos/Listar?msg=$alert");
        die();
    }
    ...
    {   
        $arrData = $this->model->selectSolicitudesEstudiantesAntiguosInactivos();
        for($i=0; $i<count($arrData); $i++)
        {   if($arrData[$i]['status']!=1)
            {
                $arrData[$i]['status'] = '<span class="badge badge-danger">Inactivo</span>'; 
